  ...
  return (
    <aside className="h-screen w-64 bg-gradient-to-b from-blue-900 to-blue-600 text-white flex flex-col py-8 px-4 shadow-xl">
      <div className="flex flex-col items-center mb-10">
        ...
        <h1 className="text-xl lg:text-2xl font-extrabold tracking-tight text-center">Job Tracker</h1>
        {user && (
          <p className="text-xs lg:text-sm text-blue-200 mt-2 text-center px-2">
      ...
      <nav className="flex flex-col gap-2">
        {navItems.map(item => (
          <Link
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-all duration-200 hover:bg-blue-800/80 hover:scale-[1.03] ${location.pathname === item.path ? 'bg-white text-blue-800 shadow font-bold' : 'bg-blue-700/0'}`}
          >
            {getIcon(item.icon)}
